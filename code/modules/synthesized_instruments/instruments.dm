...
/datum/instrument/proc/create_full_sample_deviation_map()
	...
	for (var/indx1=1, indx1<=length(delta_1)-1, indx1++)
		...
		for (var/key = from_key, key<=pivot, key++)	src.sample_map[global.musical_config.n2t(key)] = new /datum/sample_pair(sample1, key-from_key) // [55+56] / 2 -> 55.5 -> 55 so no changes will occur
		for (var/key = pivot+1, key<=to_key, key++)	src.sample_map[global.musical_config.n2t(key)] = new /datum/sample_pair(sample2, key-to_key)
	...
	for (var/key=0, key<first_key, key++) src.sample_map[global.musical_config.n2t(key)] = new /datum/sample_pair(first_sample, key-first_key)
	for (var/key=last_key, key<=127, key++)	src.sample_map[global.musical_config.n2t(key)] = new /datum/sample_pair(last_sample,  key-last_key)
	return src.samples